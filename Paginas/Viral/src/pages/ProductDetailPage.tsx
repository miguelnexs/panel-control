...
const ProductDetailPage = () => {
  ...
  // const { toggleWishlist, isInWishlist } = useWishlist(); // Wishlist not implemented in Viral yet
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center text-sm text-muted-foreground mb-8">
            ...
            {product.category_name && (
              <>
                <Link to={`/?category=${product.category}`} className="hover:text-primary transition-colors">{product.category_name}</Link>
                <span className="mx-2">/</span>
              </>
            )}
              ...
              {allImages.length > 1 && (
                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                  {allImages.map((img, idx) => (
                    ...
                    >
                      <img src={img as string} alt={`${product.name} ${idx}`} className="w-full h-full object-contain p-1" />
                    </button>
                  ))}
                </div>
              )}
