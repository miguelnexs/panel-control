...
function createWindow(): BrowserWindow {
  ...
  // Handler for Google Login via System Browser
  ipcMain.handle('start-google-auth', async (_, clientId: string) => {
    return new Promise((resolve, reject) => {
      const server = http.createServer((req, res) => {
        const reqUrl = new URL(req.url || '', `http://${req.headers.host}`);
        
        // Step 1: Handle Redirect with Token (Implicit Flow callback)
        // Since hash fragments are not sent to server, we serve a page to extract them
