...
const WebPageManager: React.FC<WebPageManagerProps> = ({ token, apiBase: rawApiBase, adminId = '', role, setView, setProductEditing, forcedTab, hideTabs }) => {
  ...
  const toggleVisible = (product: Product, isVisible: boolean) => {
    setVisible(prev => ({ ...prev, [product.id]: isVisible }));
          ...
          <div className="p-6 space-y-6">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              ...
              {Object.keys(productChanges).length > 0 && (
                <button 
                  onClick={saveProductsChanges}
                  disabled={savingChanges}
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-emerald-900/20 flex items-center gap-2 animate-in zoom-in duration-300"
                >
                  {savingChanges ? <RefreshCw className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                  Guardar Cambios Web ({Object.keys(productChanges).length})
                </button>
              )}
            </div>
            ...
            {totalProducts > pageSize && (
              <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-gray-800">
                <div className="text-sm text-gray-500">
                  Página <span className="font-medium text-gray-900 dark:text-white">{page}</span> — <span className="font-medium">{totalProducts}</span> productos en total
                </div>
                <div className="flex items-center gap-2">
                  <button
                    disabled={page === 1}
                    onClick={() => setPage(p => Math.max(1, p - 1))}
                    className="px-4 py-2 text-sm font-medium bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                    Anterior
                  </button>
                  <button
