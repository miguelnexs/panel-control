                      ...
                      <button 
                        onClick={async () => { 
                          ...
                          try { 
                            ...
                            // Orders
                            const res = await fetch(`${apiBase}/clients/orders/${c.id}/`, { headers: authHeaders(token) }); 
                            const data = await res.json(); 
                            if (res.ok) { setViewOrders(data.orders || []); } 
                            
                            // Services
                            const resServices = await fetch(`${apiBase}/services/?client=${c.id}&page_size=20`, { headers: authHeaders(token) });
                            const dataServices = await resServices.json();
                            if (resServices.ok) { 
                              const results = Array.isArray(dataServices.results) ? dataServices.results : (Array.isArray(dataServices) ? dataServices : []);
                              setViewServices(results); 
                            }
                          ...
                          <div key={s.id} className="bg-white dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700 rounded-xl p-4 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
                            ...
                            <div className="flex items-center justify-between text-xs text-gray-500">
                              <div className="flex items-center gap-2">
                                ...
                                <span>Entrada: {s.entry_date}</span>
                              </div>
                              {s.third_party_provider && (
                                <div className="flex items-center gap-2">
                                  <User className="w-3 h-3" />
                                  <span>{s.third_party_provider}</span>
                                </div>
                              )}
                            </div>
                          </div>
