...
const StatCardV2: React.FC<{ 
  ...
  <div className="relative overflow-hidden rounded-2xl bg-gray-900 border border-gray-800 p-6 transition-all hover:border-gray-700 hover:shadow-lg hover:shadow-black/50 group">
    ...
    <div className="relative z-10">
      ...
      {trend && (
        <div className={`mt-2 flex items-center text-xs font-medium ${trend === 'up' ? 'text-emerald-400' : trend === 'down' ? 'text-rose-400' : 'text-gray-400'}`}>
...
const SystemHealthCard: React.FC<{ label: string; status: 'healthy' | 'warning' | 'error'; value: string; icon: React.ElementType }> = ({ label, status, value, icon: Icon }) => (
  <div className="flex items-center justify-between p-4 rounded-xl bg-gray-800/50 border border-gray-800">
    <div className="flex items-center gap-3">
      <div className={`p-2 rounded-lg ${status === 'healthy' ? 'bg-emerald-500/10 text-emerald-500' : status === 'warning' ? 'bg-amber-500/10 text-amber-500' : 'bg-rose-500/10 text-rose-500'}`}>
        <Icon size={18} />
      </div>
      <div>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl bg-gray-900 border border-gray-800 p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold text-white">Actividad de la Plataforma</h3>
            <select className="bg-gray-800 border-none text-xs text-gray-400 rounded-lg focus:ring-0">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl bg-gray-900 border border-gray-800 p-6 h-full flex flex-col">
            ...
            <div className="mt-6 pt-6 border-t border-gray-800">
              <h4 className="text-sm font-medium text-gray-400 mb-3">Rendimiento de Consultas</h4>
              <div className="h-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <tbody className="divide-y divide-gray-800">
                {topProducts.map((product, idx) => (
                  <tr key={idx} className="hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-400 text-right">{product.qty}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="inline-flex items-center px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-xs">
        ...
        <div className="rounded-2xl bg-gray-900 border border-gray-800 p-6">
          <h3 className="text-lg font-semibold text-white mb-6">Resumen de Pedidos</h3>
          ...
          <div className="mt-8 p-4 rounded-xl bg-gray-800/50 border border-gray-700/50">
            <div className="flex items-start gap-3">
              <div className="p-2 bg-blue-500/10 rounded-lg text-blue-400">
                <Activity size={20} />
              </div>
              <div>
                <h4 className="text-sm font-medium text-white">Insight IA</h4>
