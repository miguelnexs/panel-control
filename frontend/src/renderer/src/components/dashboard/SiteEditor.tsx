...
const SiteEditor: React.FC<SiteEditorProps> = ({ token, siteUrl, apiBase, templateId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 dark:bg-gray-950 z-50 flex flex-col animate-in fade-in duration-300">
      {/* Top Bar */}
      <div className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-4">
          <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
            <X size={20} />
          </button>
          <div className="flex flex-col">
            <h1 className="text-gray-900 dark:text-white font-medium text-sm">Editor del Sitio</h1>
        ...
        <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
          <button 
            ...
            className={`p-2 rounded-md transition-all ${previewDevice === 'desktop' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
          >
            <Monitor size={18} />
          </button>
          <button 
            onClick={() => setPreviewDevice('mobile')}
            className={`p-2 rounded-md transition-all ${previewDevice === 'mobile' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'}`}
          >
            <Smartphone size={18} />
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Sidebar Controls */}
        <div className="w-80 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col overflow-y-auto">
          ...
          <div className="border-b border-gray-200 dark:border-gray-800">
            ...
            >
              <div className="flex items-center gap-3 text-gray-900 dark:text-white font-medium text-sm">
            ...
            {activeSection === 'general' && settings && (
              <div className="p-4 space-y-4 bg-gray-50 dark:bg-gray-900/50 animate-in slide-in-from-top-2 duration-200">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <textarea 
                    ...
                    rows={3}
                    className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:outline-none resize-none"
                  />
                </div>
              </div>
            )}
          ...
          <div className="border-b border-gray-200 dark:border-gray-800">
            ...
            {activeSection === 'colors' && settings && (
              <div className="p-4 space-y-4 bg-gray-50 dark:bg-gray-900/50 animate-in slide-in-from-top-2 duration-200">
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Color Principal</label>
                  <div className="flex items-center gap-2">
                    <input 
                      type="color" 
                      value={settings.primary_color}
                      onChange={(e) => updateSetting('primary_color', e.target.value)}
                      className="h-9 w-9 rounded-lg bg-transparent border border-gray-200 dark:border-gray-700 cursor-pointer"
                    />
                    <input 
                      type="text" 
                      value={settings.primary_color}
                      onChange={(e) => updateSetting('primary_color', e.target.value)}
                      className="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:outline-none"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Color Secundario</label>
                  <div className="flex items-center gap-2">
                    <input 
                      type="color" 
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Fuente</label>
                  <select 
           ...
           <div className="border-b border-gray-200 dark:border-gray-800">
            ...
            >
              <div className="flex items-center gap-3 text-gray-900 dark:text-white font-medium text-sm">
                <ImageIcon size={18} className="text-amber-500 dark:text-amber-400" />
                <span>Banners</span>
              </div>
            ...
            {activeSection === 'banners' && (
              <div className="p-4 space-y-4 bg-gray-50 dark:bg-gray-900/50 animate-in slide-in-from-top-2 duration-200">
                {banners.map(banner => (
                  <div key={banner.id} className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700 group relative">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded-md overflow-hidden">
                        {banner.image ? <img src={banner.image} alt="" className="w-full h-full object-cover" /> : <ImageIcon className="w-5 h-5 m-auto mt-2.5 text-gray-400 dark:text-gray-500" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{banner.title || 'Sin título'}</p>
                        <p className="text-xs text-gray-500">{banner.active ? 'Activo' : 'Inactivo'}</p>
                      </div>
                      <button 
                        onClick={() => deleteBanner(banner.id)}
                        className="p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-rose-500 dark:hover:text-rose-400 transition-colors"
                      >
                        <Trash size={14} />
                      </button>
                    </div>
                ...
                <button 
                  ...
                  className="w-full py-2 border border-dashed border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:border-gray-400 dark:hover:border-gray-600 transition-colors flex items-center justify-center gap-2"
                >
                  <Plus size={16} />
        ...
        {/* Preview Area */}
        <div className="flex-1 bg-gray-100 dark:bg-gray-950 flex items-center justify-center relative p-8">
          <div 
            className={`transition-all duration-500 ease-in-out bg-white shadow-2xl overflow-hidden ${
              previewDevice === 'mobile' 
                ...
                : 'w-full h-full rounded-xl border border-gray-200 dark:border-gray-800'
            }`}
          >
            {fullUrl ? (
              <iframe 
