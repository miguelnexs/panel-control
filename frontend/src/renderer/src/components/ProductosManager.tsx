...
const ProductosManager: React.FC<ProductosManagerProps> = ({ token, apiBase, onCreate, onEdit, canCreate, canEdit, canDelete, canReorder }) => {
  ...
  const confirmDeleteProduct = async () => {
    ...
    setMsg(null);
    try {
      const res = await fetch(`${apiBase}/products/${deletingProduct.id}/`, { method: 'DELETE', headers: authHeaders(token) });
      if (!res.ok) {
        const data = await res.json();
                        ...
                        {viewColors.map((c) => (
                          <div key={c.id} className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-400">Stock: {c.stock}</div>
                            </div>
                          </div>
                        ))}
                      </div>
