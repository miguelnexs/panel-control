                ...
                <div className="flex items-center gap-2">
                  {settings.logo ? (
                    ...
                    <EditableText 
                      ...
                      field="company_name" 
                      disableDrag={true} 
                      className="text-xl font-black tracking-tight text-custom-primary"
                    />
                  )}
                </div>
              </div>
              <nav className="hidden md:flex items-center gap-8 text-sm font-semibold text-gray-600">
                {(data.nav_links || []).map((link: string, i: number) => (
                  <a key={i} href="#" className="hover:text-custom-primary transition-colors">{link}</a>
                ))}
              </nav>
