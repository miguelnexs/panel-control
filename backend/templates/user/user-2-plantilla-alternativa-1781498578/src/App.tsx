                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          ...
                          reader.onload = (event) => {
                            const base64 = event.target?.result as string;
                            if (base64) {
                              window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'bg_video', value: base64 }, '*');
                            }
                            setIsUploadingVideo(false);
                          };
                          reader.onerror = () => {
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      reader.onload = (event) => {
                        const base64 = event.target?.result as string;
                        if (base64) {
                          window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'bg_image', value: base64 }, '*');
                        }
                      };
                      reader.readAsDataURL(file);
                    }
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              reader.onload = (event) => {
                                const base64 = event.target?.result as string;
                                if (base64) {
                                  window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'video', value: base64 }, '*');
                                }
                                setIsUploadingVideo(false);
                              };
                              reader.onerror = () => {
      ...
      {isUploadingVideo && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-gray-100 text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-black text-gray-900">Procesando Video...</h3>
              <p className="text-xs text-gray-500 max-w-xs mx-auto">
                Estamos optimizando y cargando tu video para la página web. Por favor espera un momento.
              </p>
            </div>
