...
class AppSettings(models.Model):
    ...
    # Service printer settings
    service_printer_type = models.CharField(max_length=20, blank=True, default='system')
    service_printer_name = models.CharField(max_length=120, blank=True, default='')
    service_paper_width_mm = models.PositiveIntegerField(default=58)
    service_auto_print = models.BooleanField(default=True)
    service_receipt_footer = models.TextField(blank=True, default='')

    whatsapp_config = models.JSONField(default=dict, blank=True)
    google_config = models.JSONField(default=dict, blank=True)
    alegra_config = models.JSONField(default=dict, blank=True)
