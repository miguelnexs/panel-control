            ...
            try:
                for item in items_data:
                    ...
                    if c_id or v_id:
                        ...
                        if not sku_obj:
                            raise Exception(f"Combinación no disponible para {product.name}")
                        if sku_obj.stock < qty:
                            raise Exception(f"Stock insuficiente para {product.name} ({sku_obj.stock} disponibles)")
                        color = sku_obj.color
                    else:
                    ...
                    if sku_obj:
                        ...
                        deducted_items.append({'type': 'sku', 'obj': sku_obj, 'qty': qty})
                    else:
