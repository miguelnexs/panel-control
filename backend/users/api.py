...
class RegisterTenantSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        # 3. Obtener plan - Forzar 'medium' (Intermedio) para el trial de 15 días
        try:
            plan = SubscriptionPlan.objects.get(code='medium')
        except SubscriptionPlan.DoesNotExist:
            plan = SubscriptionPlan.objects.get(code=validated_data.get('plan_code', 'basic'))
...
class UsersView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Si es super_admin, puede ver todo o filtrar por tenant
            if role == 'super_admin' and scope != 'tenant':
                ...
                # Para admin y employee, solo mostramos los usuarios de SU propio tenant
                user_tenant = _get_user_tenant(request.user)
                if user_tenant:
                    # Incluimos tanto al admin como a los empleados del mismo tenant
...
class UserDetailView(APIView):
    ...
    def delete(self, request, user_id):
        ...
        # Prevención: No eliminar a sí mismo
        if target.id == request.user.id:
            return Response({'detail': 'No puedes eliminar tu propia cuenta.'}, status=status.HTTP_403_FORBIDDEN)

        # Lógica por rol
        if requester_role == 'super_admin':
            # Super admin puede eliminar cualquier cuenta excepto otros super admins y la propia
            if target_role == 'super_admin':
                return Response({'detail': 'No se puede eliminar a otro Super Administrador.'}, status=status.HTTP_403_FORBIDDEN)
        else:
            # Admin regular
            if target_role != 'employee':
