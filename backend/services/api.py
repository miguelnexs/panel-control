...
class ServiceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'third_party_provider', 'third_party_cost', 'value', 'status', 'delivery_note', 'active', 'created_at',
    ...
    def validate_value(self, value):
        ...
        # Check for decimal places safely
    ...
    def to_representation(self, instance):
        try:
            ...
            # Safe worker name extraction
            worker_name = None
            if instance.worker:
                try:
                    worker_name = instance.worker.get_full_name() or instance.worker.username
                except Exception:
                    worker_name = getattr(instance.worker, 'username', 'Desconocido')
            data['worker_name'] = worker_name
            
            return data
        except Exception as e:
            # Fallback to basic representation if something fails
    ...
    def create(self, validated_data):
        try:
            ...
            if not client:
                ...
                validated_data.pop('client_type', None)
                
            return super().create(validated_data)
        except Exception as e:
            raise serializers.ValidationError(f"Error en creación: {str(e)}")
    ...
    def perform_create(self, serializer):
        try:
            ...
            log_activity(
                ...
                resource_id=str(getattr(service, 'id', '')),
                message=f'Servicio creado: {getattr(service, "name", "")}',
                metadata={
                    'name': str(getattr(service, 'name', '')),
                    'status': str(getattr(service, 'status', '')),
                    'value': str(getattr(service, 'value', '0'))
                },
                request=self.request,
            )
        except Exception as e:
            from rest_framework.exceptions import ValidationError
            raise ValidationError({'detail': f"Error interno al crear el servicio: {str(e)}"})
...
class ServiceDeliverView(APIView):
    ...
    def post(self, request, pk):
        ...
        # Capture delivery note and updated value if provided
        note = request.data.get('note')
        if note:
            service.delivery_note = note
            
        value = request.data.get('value')
        if value is not None:
            try:
                service.value = Decimal(str(value))
