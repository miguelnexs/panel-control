...
from products.models import Product, Category, ProductColor, ProductVariant, ProductSKU
...
class TemplateListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Template.objects.filter(is_active=True, is_personal=False).order_by('-created_at')
...
class WebSettingsView(views.APIView):
    ...
    def get(self, request):
        ...
        # Include flags from TenantConfiguration
        try:
            from users.models_tenant_config import TenantConfiguration
            if user_tenant:
                config = TenantConfiguration.objects.filter(tenant=user_tenant).first()
                data['enable_web_store'] = config.enable_web_store if config else False
                data['enable_web_sync'] = config.enable_web_sync if config else True
                data['enable_web_advertising'] = config.enable_web_advertising if config else True
            else:
                data['enable_web_store'] = False
                data['enable_web_sync'] = True
                data['enable_web_advertising'] = True
        except Exception:
            data['enable_web_store'] = False
            data['enable_web_sync'] = True
            data['enable_web_advertising'] = True
...
class TestGoogleConfigView(views.APIView):
    ...
    def post(self, request):
        ...
        if email or app_password:
            ...
            import smtplib
            try:
                # Default to Gmail but allow custom host/port for corporate emails
                smtp_host = request.data.get('smtp_host')
                smtp_port = request.data.get('smtp_port')
                use_tls = request.data.get('smtp_use_tls', True)
                ...
                if not smtp_host:
                    try:
                        profile = getattr(request.user, 'profile', None)
                        user_tenant = profile.tenant if profile else None
                        ws = AppSettings.objects.filter(tenant=user_tenant).first() if user_tenant else AppSettings.objects.first()
                        if ws and ws.google_config:
                            smtp_host = ws.google_config.get('smtp_host', 'smtp.gmail.com')
                            smtp_port = ws.google_config.get('smtp_port', 587)
                            use_tls = ws.google_config.get('smtp_use_tls', True)
                            use_ssl = ws.google_config.get('smtp_use_ssl', False)
                    except Exception:
                        pass
                ...
                return Response({'detail': 'Conexión SMTP exitosa. Credenciales válidas.'})
                 ...
                 return Response({'detail': 'Error de autenticación: Correo o contraseña incorrectos.'}, status=status.HTTP_401_UNAUTHORIZED)
            except Exception as e:
                 return Response({'detail': f'Error de conexión SMTP ({smtp_host}:{smtp_port}): {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class VisibleProductStatusListView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    def get(self, request):
        try:
            profile = getattr(request.user, 'profile', None)
        ...
        if user_tenant:
            prods = Product.objects.filter(tenant=user_tenant).order_by('-created_at')
        else:
            if role == 'super_admin':
                prods = Product.objects.all().order_by('-created_at')
            elif role == 'admin':
                prods = Product.objects.filter(tenant__isnull=True).order_by('-created_at')
            else:
                prods = Product.objects.none()
        
        prods = prods.only('id', 'name', 'sku', 'price', 'image')
        
        # Get all visible entries at once
        visible_map = {v.product_id: v for v in VisibleProduct.objects.filter(product__in=prods)}
        
        result = []
        for p in prods:
            v = visible_map.get(p.id)
            result.append({
                'id': p.id,
                'name': p.name,
                'sku': p.sku,
                ...
                'visible': bool(v and v.active),
                'position': int(getattr(v, 'position', 0) or 0),
            })
        return Response(result)


class VisibleProductsBulkUpdateView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    def post(self, request):
        data = request.data or {}
        items = data.get('items', []) # List of {product_id, visible, position}
        ...
        with transaction.atomic():
            for it in items:
                ...
                obj, _ = VisibleProduct.objects.get_or_create(product_id=pid)
...
class VisibleCategoriesBulkUpdateView(views.APIView):
    permission_classes = [permissions.IsAuthenticated]
    def post(self, request):
        data = request.data or {}
        items = data.get('items', []) # List of {category_id, visible, position}
        ...
        with transaction.atomic():
            for it in items:
                ...
                obj, _ = VisibleCategory.objects.get_or_create(category_id=cid)
                if visible is not None:
                    obj.active = bool(visible)
                if position is not None:
                    obj.position = int(position)
...
class PublicPortalView(views.APIView):
    ...
    def get(self, request):
        ...
        cats = Category.objects.filter(tenant=tenant, active=True) if tenant else Category.objects.none()
        if tenant and cat_ids:
            # If there's a specific selection, respect it
            cats = cats.filter(id__in=cat_ids)
        # If no specific selection (cat_ids empty), 'cats' already has all active ones for tenant
        ...
        return Response({
            ...
            'policy': PolicySerializer(policy).data,
            'products': ProductSerializer(prods, many=True, context={'request': request}).data,
            'categories': CategorySerializer(cats, many=True).data if cats.exists() else [],
        })
...
class PublicProductsView(views.APIView):
    ...
    def get(self, request):
        ...
        if prods.exists():
            ...
            if is_sale:
                prods = prods.filter(is_sale=(is_sale.lower() == 'true'))
            
            is_new = request.query_params.get('is_new')
            if is_new:
                prods = prods.filter(is_new=(is_new.lower() == 'true'))

            is_featured = request.query_params.get('is_featured')
            if is_featured:
                prods = prods.filter(is_featured=(is_featured.lower() == 'true'))
        ...
        try:
            with transaction.atomic():
                ...
                for it in items:
                    ...
                    sku_obj = None
                    # Stock handling
                    if color_id or variant_id:
                        sku_obj = ProductSKU.objects.filter(
                            product=product,
                            color_id=color_id,
                            variant_id=variant_id
                        ).first()
                        if not sku_obj:
                            raise ValueError('Combinación de producto no existe')
                        if sku_obj.stock < qty:
                            raise ValueError(f'Stock insuficiente para esta combinación ({sku_obj.stock} disponibles)')
                        color = sku_obj.color
                        
                        sku_obj.stock -= qty
                        sku_obj.save(update_fields=['stock'])
                    else:
...
class VisibleCategoryStatusListView(views.APIView):
    ...
    def get(self, request):
        ...
        cats = cats.only('id', 'name', 'description', 'image')
        
        # Optimize with a map to avoid N+1 queries
        visible_map = {vc.category_id: vc for vc in VisibleCategory.objects.filter(category__in=cats)}
        
        result = []
        for c in cats:
            vc = visible_map.get(c.id)
            result.append({
                'id': c.id,
                'name': c.name,
...
class PublicCategoriesView(views.APIView):
    ...
    def get(self, request):
        ...
        # Get categories for this tenant that are active
        cats = Category.objects.filter(tenant=tenant, active=True).order_by('position', 'name')
        
        # Check if there's a specific "Visible" selection
        vis_ids = list(VisibleCategory.objects.filter(category__tenant=tenant, active=True).values_list('category_id', flat=True))
        if vis_ids:
            # If user has specifically selected some categories to be visible, only show those
            cats = cats.filter(id__in=vis_ids)

        def get_img(c):
            try:
                if not c.image: return None
                url = c.image.url
