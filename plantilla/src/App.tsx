...
const EditableText = ({
  ...
  const currentTextAlign = styles.textAlign || customStyleProps.textAlign || 'inherit';
  const currentAlignItems = styles.alignItems || customStyleProps.alignItems || 'inherit';
  const currentFontWeight = styles.fontWeight || customStyleProps.fontWeight || 'inherit';
  const currentFontStyle = styles.fontStyle || customStyleProps.fontStyle || 'inherit';
  const currentTextDecoration = styles.textDecoration || customStyleProps.textDecoration || 'inherit';
  const currentBackgroundColor = styles.backgroundColor || customStyleProps.backgroundColor || 'transparent';

  const getAlignmentClasses = () => {
    if (disableDrag) {
      if (currentTextAlign === 'left') return 'text-left';
      if (currentTextAlign === 'right') return 'text-right';
      if (currentTextAlign === 'center') return 'text-center';
      return '';
    }
        ...
        (() => {
          ...
          return (
            <Tag
              ...
              className={`outline-none hover:outline-dashed hover:outline-1 hover:outline-gray-400 focus:outline-solid focus:ring-2 focus:ring-[var(--primary)] rounded px-1 -mx-1 transition-all ${finalClassName} ${getAlignmentClasses()}`}
              style={{
                 display: disableDrag ? undefined : 'flex',
                 alignItems: disableDrag ? undefined : (currentAlignItems !== 'inherit' && currentAlignItems ? currentAlignItems : 'center'),
                 justifyContent: disableDrag ? undefined : (currentTextAlign === 'left' ? 'flex-start' : currentTextAlign === 'right' ? 'flex-end' : 'center'),
                 fontSize: currentFontSize ? `${currentFontSize}px` : undefined,
                 color: currentColor !== 'inherit' ? currentColor : undefined,
                 backgroundColor: currentBackgroundColor !== 'transparent' ? currentBackgroundColor : undefined,
                 fontFamily: currentFontFamily !== 'inherit' ? currentFontFamily : undefined,
...
const SectionControls = ({ section, isSelected, onConfig }: { section: string, isSelected: boolean, onConfig?: () => void }) => {
    ...
    <div className="absolute top-2 right-2 flex gap-1 z-50">
       {onConfig && (
         <button 
           ...
           className="px-2.5 py-1 bg-white border border-gray-300 rounded shadow-md hover:bg-gray-100 text-gray-700 text-xs font-bold transition-colors flex items-center gap-1"
...
export default function App() {
  ...
  const [heroIdForModal, setHeroIdForModal] = useState<string | null>(null);
  const [isUploadingVideo, setIsUploadingVideo] = useState(false);
  const [videoUploadProgress, setVideoUploadProgress] = useState(0);
  const [uploadingText, setUploadingText] = useState('Procesando Video...');
  const authTokenRef = useRef<string | null>(null);
  const [authToken, setAuthToken] = useState<string | null>(null);
  const [dbProducts, setDbProducts] = useState<any[]>([]);

  // Expose authToken globally so child route pages (like /productos) can access it
  if (typeof window !== 'undefined') {
    (window as any).__plantillaAuthToken = authToken;
  }
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (type === "AUTH_TOKEN") {
        authTokenRef.current = payload?.token || null;
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const isProductsPage = window.location.pathname === '/productos';
        const url = `${apiBase}/webconfig/public/products/?site=${window.location.hostname}${isProductsPage ? '' : '&page_size=4'}`;
    ...
    switch (type) {
          ...
          <header key={id} data-section={id} style={sectionStyle(id, data)} className="relative sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-gray-200 shadow-sm">
            ...
            <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
              ...
              <nav className="hidden md:flex items-center gap-6 text-sm font-semibold text-gray-600">
                {(Array.isArray(data.nav_links) ? data.nav_links : [
                  { label: "Inicio", url: "/" },
                  ...
                  return (
                    <a
                      ...
                      onClick={(e) => {
                        // Prevent navigation if we are inside the editor iframe
                        if (window.parent !== window) {
                          e.preventDefault();
                          // No post message here because editing label/url is now done purely from PropertiesPanel
                        }
                      }}
                    >
                      {link.label}
      ...
      case 'hero': {
        ...
        if (data.bg_type === 'image' && data.bg_image) {
          bgStyle.backgroundImage = `url(${data.bg_image})`;
          bgStyle.backgroundSize = 'cover';
          bgStyle.backgroundPosition = 'center';
        } else if (data.bg_type === 'gradient') {
          bgStyle.background = data.bg_gradient || 'linear-gradient(135deg, var(--primary) 0%, var(--secondary) 100%)';
        } else if (data.bg_type === 'color') {
          bgStyle.backgroundColor = data.bg_color || '#f9fafb';
        }

        const isBgDark = data.bg_type === 'image' || data.bg_type === 'video';

        const renderBgVideo = () => {
          if (data.bg_type !== 'video' || !data.bg_video) return null;
          const ytId = getYouTubeId(data.bg_video);
          return (
            <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
              {ytId ? (
                <iframe
                  src={`https://www.youtube.com/embed/${ytId}?autoplay=1&mute=1&loop=1&playlist=${ytId}&controls=0&showinfo=0&rel=0&playsinline=1&enablejsapi=1`}
        ...
        if (data.layout === 'full') {
          return (
            <section 
              key={id} 
              data-section={id} 
              style={{ ...sectionStyle(id, data), ...bgStyle }} 
              className="relative py-28 px-4 overflow-hidden border-b border-gray-100 flex items-center justify-center min-h-[65vh] text-center"
            >
              {renderBgVideo()}
              <SectionControls 
                section={id} 
                isSelected={isSelected} 
                onConfig={() => { setHeroIdForModal(id); setHeroModalOpen(true); }} 
              />
              
              {isBgDark && (
                <div 
                  className="absolute inset-0 bg-black z-0" 
                  style={{ opacity: data.bg_overlay_opacity !== undefined ? data.bg_overlay_opacity / 100 : 0.4 }} 
                />
              )}

              <div className={`relative z-10 max-w-4xl mx-auto space-y-6 ${isBgDark ? 'text-white' : 'text-gray-900'}`}>
                {(data.show_badge !== false) && (
                  <EditableText 
                    ...
                    className={`inline-block text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider ${isBgDark ? 'bg-white/20 text-white' : 'bg-custom-primary/10 text-custom-primary'}`} 
                  />
                )}
                ...
                <h1 className="text-4xl sm:text-5xl md:text-6xl font-black leading-tight tracking-tight">
                  ...
                  <EditableText 
                    ...
                    field="company_name" 
                    disableDrag={true} 
                    className={isBgDark ? 'text-amber-400' : 'text-custom-primary'}
                  />
                </h1>
                ...
                <EditableText 
                  ...
                  className={`text-lg leading-relaxed max-w-2xl mx-auto block ${isBgDark ? 'text-white/95' : 'text-gray-600'}`} 
                />
                
                <div className="flex flex-wrap justify-center gap-4 pt-4">
                  {(data.show_btn_primary !== false) && (
        ...
        return (
          <section 
            ...
            data-section={id} 
            style={{ ...sectionStyle(id, data), ...bgStyle }} 
            className="relative py-20 px-4 overflow-hidden border-b border-gray-100"
          >
            {renderBgVideo()}
            <SectionControls 
              section={id} 
            ...
            {isBgDark && (
              <div 
                className="absolute inset-0 bg-black z-0" 
                style={{ opacity: data.bg_overlay_opacity !== undefined ? data.bg_overlay_opacity / 100 : 0.2 }} 
              />
            )}

            <div className={`relative z-10 max-w-7xl mx-auto grid md:grid-cols-2 items-center gap-12 ${isBgDark ? 'text-white' : 'text-gray-900'}`}>
              <div className="space-y-6">
                ...
                <EditableText 
                  ...
                  position={data.positions?.description}
                  styles={data.styles?.description}
                  className={`text-lg leading-relaxed max-w-lg block ${isBgDark ? 'text-white/90' : 'text-gray-600'}`} 
                />
              ...
              <DraggableContainer section={id} field="image_hero" position={data.positions?.image_hero} className="w-full">
                <div 
                  className="relative aspect-square md:aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center bg-gray-100 border border-gray-200 group cursor-pointer"
                  onClick={(e) => {
                    // Only open uploader if clicked element isn't an iframe or video controls
                    if (data.hero_media_type !== 'video') {
                      e.stopPropagation();
                      window.parent.postMessage({ type: 'CANVAS_IMAGE_CLICKED', section: id }, '*');
                    }
                  }}
                >
                  {data.hero_media_type === 'video' && data.video ? (
                    getYouTubeId(data.video) ? (
                      <iframe
                        src={`https://www.youtube.com/embed/${getYouTubeId(data.video)}?autoplay=0&mute=0`}
                        className="w-full h-full border-0 absolute inset-0 rounded-3xl z-10"
                        allowFullScreen
                        title="Hero Video"
                      />
                    ) : (
                      <video src={data.video} controls className="w-full h-full object-cover absolute inset-0 rounded-3xl z-10" />
                    )
                  ) : data.image ? (
                    <img src={data.image} alt="Hero" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 z-10" />
                  ) : (
                    <div className="w-full h-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 transition-colors z-10">
          ...
          <section key={id} data-section={id} style={sectionStyle(id, data)} className="relative py-20 bg-gray-50 px-4 border-b border-gray-100">
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {dbProducts.length > 0 ? (
                  dbProducts.slice(0, data.limit || 4).map((prod) => (
                    <div key={prod.id} className="bg-white rounded-3xl p-4 shadow-sm hover:shadow-xl transition-all group">
                      <div className="aspect-[4/5] rounded-2xl bg-gray-100 mb-4 relative overflow-hidden flex items-center justify-center cursor-pointer group/img">
                        {prod.image ? (
                          <img src={prod.image.startsWith('http') ? prod.image : `${window.location.hostname === "localhost" ? "http://localhost:8000" : ""}${prod.image.startsWith('/') ? '' : '/'}${prod.image}`} alt={prod.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ) : (
      ...
      {heroModalOpen && heroIdForModal && (
        ...
        >
          ...
          >
            {/* Modal Header */}
            <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
              <h3 className="font-bold text-gray-900 text-base">Diseño del Hero Principal</h3>
              <button 
                onClick={() => setHeroModalOpen(false)}
                className="text-gray-400 hover:text-gray-600 text-lg font-bold"
              >
            ...
            <div className="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              {/* Layout selector */}
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Distribución (Diseño)</h4>
                <div className="grid grid-cols-2 gap-2 bg-gray-100 p-1 rounded-xl">
                  <button
                    type="button"
                    onClick={() => {
                      window.parent.postMessage({
                        type: 'INLINE_TEXT_UPDATE',
                        section: heroIdForModal,
                        field: 'layout',
                        value: 'split'
                      }, '*');
                    }}
                    className={`py-2 text-xs font-bold rounded-lg transition-all ${sectionData[heroIdForModal]?.layout !== 'full' ? 'bg-white text-custom-primary shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                  >
                    Dividido (Imagen Derecha)
                  </button>
                  <button
                    type="button"
                    onClick={() => {
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Fondo del Banner</h4>
                  ...
                  {['color', 'gradient', 'image', 'video'].map((t) => (
                    <button
                      ...
                      className={`py-1.5 text-[9px] uppercase font-black rounded-lg transition-all ${
                        (sectionData[heroIdForModal]?.bg_type || 'color') === t
                          ? 'bg-white text-custom-primary shadow-sm' 
                          : 'text-gray-500 hover:text-gray-900'
                      }`}
                    >
                      {t === 'color' ? 'Color' : t === 'gradient' ? 'Grad.' : t === 'image' ? 'Img' : 'Vid'}
                    </button>
                  ))}
                </div>

                {/* Background Details Input */}
                {(sectionData[heroIdForModal]?.bg_type === 'image') ? (
                  <div className="space-y-3 pt-1">
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      {sectionData[heroIdForModal]?.bg_image ? (
                        <img src={sectionData[heroIdForModal].bg_image} className="w-12 h-12 object-cover rounded-lg" />
                      ) : (
                        <div className="w-12 h-12 bg-gray-200 rounded-lg flex items-center justify-center text-[10px] text-gray-400 font-bold">Sin Imagen</div>
                      )}
                      <button
                        type="button"
                        onClick={() => document.getElementById('hero-bg-upload')?.click()}
                        className="px-3 py-1.5 bg-custom-primary text-white text-xs font-bold rounded-lg"
                      >
                        Subir Imagen
                      </button>
                      {sectionData[heroIdForModal]?.bg_image && (
                        <button
                          type="button"
                          onClick={() => {
                            window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'bg_image', value: null }, '*');
                          }}
                          className="p-1 text-red-500 hover:bg-red-50 rounded"
                        >
                          Eliminar
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase">
                        ...
                        <span>{sectionData[heroIdForModal]?.bg_overlay_opacity !== undefined ? sectionData[heroIdForModal].bg_overlay_opacity : 40}%</span>
                      </div>
                      <input
                        type="range"
                        min="0"
                        max="90"
                        step="5"
                        value={sectionData[heroIdForModal]?.bg_overlay_opacity !== undefined ? sectionData[heroIdForModal].bg_overlay_opacity : 40}
                        onChange={(e) => {
                          window.parent.postMessage({
                            type: 'INLINE_TEXT_UPDATE',
                            section: heroIdForModal,
                            field: 'bg_overlay_opacity',
                            value: parseInt(e.target.value)
                          }, '*');
                        }}
                        className="w-full accent-custom-primary"
                      />
                    </div>
                  </div>
                ) : (sectionData[heroIdForModal]?.bg_type === 'video') ? (
                  <div className="space-y-3 pt-1">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-gray-400 uppercase">Enlace del Video (YouTube o MP4)</label>
                      <input
                        type="text"
                        placeholder="https://www.youtube.com/watch?v=..."
                        value={sectionData[heroIdForModal]?.bg_video || ''}
                        onChange={(e) => {
                          window.parent.postMessage({
                            type: 'INLINE_TEXT_UPDATE',
                            section: heroIdForModal,
                        ...
                        className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:outline-none focus:border-custom-primary"
                      />
                    </div>
                    
                    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <span className="text-xs font-bold text-gray-500">Subir Video Local (.mp4)</span>
                      <button
                        type="button"
                        onClick={() => document.getElementById('hero-bg-video-upload')?.click()}
                        className="px-3 py-1.5 bg-custom-primary text-white text-xs font-bold rounded-lg"
                      >
                        Subir MP4
                      </button>
                          ...
                          xhr.onload = () => {
                            if (xhr.status === 201) {
                              try {
                                const resp = JSON.parse(xhr.responseText);
                                if (resp.url) {
                                  window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'bg_video', value: resp.url }, '*');
                                }
                              } catch (err) {
                                alert('Error al procesar la respuesta del servidor.');
                              }
                              ...
                              try {
                                const errResp = JSON.parse(xhr.responseText || '{}');
                                alert(errResp.detail || 'Error al subir el video.');
                              } catch (e) {
                ...
                ) : (sectionData[heroIdForModal]?.bg_type === 'gradient') ? (
                  <div className="space-y-2 pt-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">CSS Gradient Rule</label>
                    <input
                      type="text"
                      placeholder="linear-gradient(135deg, #10b981 0%, #f0fdf4 100%)"
                      value={sectionData[heroIdForModal]?.bg_gradient || ''}
                      onChange={(e) => {
                        window.parent.postMessage({
                          type: 'INLINE_TEXT_UPDATE',
                          section: heroIdForModal,
                      ...
                      className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono focus:outline-none focus:border-custom-primary"
                    />
                  </div>
                ) : (
                  <div className="space-y-2 pt-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase">Color de Fondo Hex/RGB</label>
                    <div className="flex gap-2">
                      <input
                        type="color"
                        value={sectionData[heroIdForModal]?.bg_color || '#f9fafb'}
                        onChange={(e) => {
                          window.parent.postMessage({
                            type: 'INLINE_TEXT_UPDATE',
                            section: heroIdForModal,
                      ...
                      <input
                        ...
                        className="flex-1 px-3 py-1 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono focus:outline-none focus:border-custom-primary"
                      />
                    </div>
                  </div>
                )}

                {/* Hidden Background Uploader */}
                <input
                  id="hero-bg-upload"
                  type="file"
                  accept="image/*"
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      xhr.onloadstart = () => {
                        setUploadingText('Procesando Imagen de Fondo...');
                        setIsUploadingVideo(true);
                        setVideoUploadProgress(0);
                      };
                      ...
                      xhr.onload = () => {
                        if (xhr.status === 201) {
                          try {
                            const resp = JSON.parse(xhr.responseText);
                            if (resp.url) {
                              window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'bg_image', value: resp.url }, '*');
                            }
                          } catch (err) {
                            alert('Error al procesar la respuesta del servidor.');
                          }
              ...
              {/* Multimedia split layout */}
              {sectionData[heroIdForModal]?.layout !== 'full' && (
                <>
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Multimedia Lado Derecho</h4>
                    
                    <div className="grid grid-cols-2 gap-2 bg-gray-100 p-1 rounded-xl">
                      <button
                        type="button"
                        onClick={() => {
                          window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'hero_media_type', value: 'image' }, '*');
                        }}
                        className={`py-1.5 text-xs font-bold rounded-lg transition-all ${sectionData[heroIdForModal]?.hero_media_type !== 'video' ? 'bg-white text-custom-primary shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                      >
                        Imagen
                      </button>
                      <button
                        type="button"
                        onClick={() => {
                          window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'hero_media_type', value: 'video' }, '*');
                        }}
                        className={`py-1.5 text-xs font-bold rounded-lg transition-all ${sectionData[heroIdForModal]?.hero_media_type === 'video' ? 'bg-white text-custom-primary shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
                      >
                        Video
                    ...
                    {sectionData[heroIdForModal]?.hero_media_type === 'video' ? (
                      <div className="space-y-3 pt-1">
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-gray-400 uppercase">Enlace del Video (YouTube o MP4)</label>
                          <input
                            type="text"
                            placeholder="https://www.youtube.com/watch?v=..."
                            value={sectionData[heroIdForModal]?.video || ''}
                            onChange={(e) => {
                              window.parent.postMessage({
                                type: 'INLINE_TEXT_UPDATE',
                                section: heroIdForModal,
                        ...
                        <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                          <span className="text-xs font-bold text-gray-500">Subir Video (.mp4)</span>
                          <button
                            type="button"
                            onClick={() => document.getElementById('hero-media-video-upload')?.click()}
                            className="px-3 py-1.5 bg-custom-primary text-white text-xs font-bold rounded-lg"
                          >
                            Subir MP4
                          </button>
                              ...
                              xhr.onload = () => {
                                if (xhr.status === 201) {
                                  try {
                                    const resp = JSON.parse(xhr.responseText);
                                    if (resp.url) {
                                      window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'video', value: resp.url }, '*');
                                    }
                                  } catch (err) {
                                    alert('Error al procesar la respuesta del servidor.');
                                  }
                      ...
                      <div className="flex flex-col gap-2 p-3 bg-gray-50 rounded-xl border border-gray-100 w-full">
                        <div className="flex items-center gap-3">
                          {sectionData[heroIdForModal]?.image ? (
                            <img src={sectionData[heroIdForModal].image} className="w-12 h-12 object-cover rounded-lg" />
                          ) : (
                            <div className="w-12 h-12 bg-gray-200 rounded-lg flex items-center justify-center text-[10px] text-gray-400 font-bold">Sin Imagen</div>
                          )}
                          <div className="flex flex-col gap-1.5">
                            <button
                              type="button"
                              onClick={() => document.getElementById('hero-media-image-upload')?.click()}
                              className="px-3 py-1.5 bg-custom-primary text-white text-xs font-bold rounded-lg text-center"
                            >
                              Subir Imagen
                            </button>
                            <button
                              type="button"
                              onClick={() => window.parent.postMessage({ type: 'CANVAS_IMAGE_CLICKED', section: heroIdForModal }, '*')}
                              className="px-3 py-1 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-xs font-bold rounded-lg text-center"
                            >
                              Elegir Galería
                            </button>
                          </div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              xhr.onloadstart = () => {
                                setUploadingText('Procesando Imagen Multimedia...');
                                setIsUploadingVideo(true);
                                setVideoUploadProgress(0);
                              };
                              ...
                              xhr.onload = () => {
                                if (xhr.status === 201) {
                                  try {
                                    const resp = JSON.parse(xhr.responseText);
                                    if (resp.url) {
                                      window.parent.postMessage({ type: 'INLINE_TEXT_UPDATE', section: heroIdForModal, field: 'image', value: resp.url }, '*');
                                    }
                                  } catch (err) {
                                    alert('Error al procesar la respuesta del servidor.');
                                  }
              ...
              {/* Elements Visibility */}
              <div className="space-y-2">
                <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Elementos Visibles</h4>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { label: "Badge", field: "show_badge", currentVal: sectionData[heroIdForModal]?.show_badge !== false },
                    { label: "Botón 1", field: "show_btn_primary", currentVal: sectionData[heroIdForModal]?.show_btn_primary !== false },
                    { label: "Botón 2", field: "show_btn_secondary", currentVal: sectionData[heroIdForModal]?.show_btn_secondary !== false },
                  ].map((item) => (
                    <label key={item.field} className="flex flex-col items-center justify-center p-2.5 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100/50">
                      <input
                        type="checkbox"
                        checked={item.currentVal}
                        onChange={(e) => {
                        ...
                        className="rounded border-gray-300 text-custom-primary focus:ring-custom-primary mb-1.5"
                      />
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-gray-100 text-center space-y-6">
            <div className="relative w-20 h-20 mx-auto flex items-center justify-center">
              ...
              <div className="absolute inset-0 rounded-full border-4 border-custom-primary border-t-transparent animate-spin" />
              <span className="text-sm font-black text-custom-primary">{videoUploadProgress}%</span>
            </div>
            <div className="space-y-2">
              <h3 className="text-lg font-black text-gray-900">{uploadingText}</h3>
              <p className="text-xs text-gray-500 max-w-xs mx-auto">
                Estamos optimizando y cargando tu archivo para la página web. Por favor espera un momento.
              </p>
            </div>
            <div className="w-full bg-gray-100 h-2 rounded-full overflow-hidden">
              <div 
                className="bg-custom-primary h-full transition-all duration-300 ease-out" 
