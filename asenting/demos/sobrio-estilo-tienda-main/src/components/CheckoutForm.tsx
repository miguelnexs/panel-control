...
const CheckoutForm = () => {
  ...
  const [mpPublicKey, setMpPublicKey] = useState('TEST-52a4805c-0268-45e0-825f-228795325791'); // Placeholder key
  ...
  useEffect(() => {
    const fetchMpKey = async () => {
      try {
        // Intentar obtener la clave pública del backend
        const response = await fetch('http://127.0.0.1:8000/webconfig/payments/');
        if (response.ok) {
          const data = await response.json();
          // Buscar el método de pago de Mercado Pago activo
        ...
        console.error('Error fetching Mercado Pago configuration:', error);
      }
    };
              ...
              <CardContent>
                <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-4">
                  ...
                  <div className={`relative flex items-center space-x-4 p-6 border-2 rounded-xl cursor-pointer transition-all duration-300 ${
                    ...
                    <RadioGroupItem value="mercadopago" id="mercadopago" className="border-2 border-blue-500 text-blue-500" />
                    <div className="flex-1">
                      <label htmlFor="mercadopago" className="flex items-center gap-4 cursor-pointer">
                        <div className={`p-3 rounded-lg transition-colors ${
                          paymentMethod === 'mercadopago' ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-600'
                        }`}>
                          <CreditCard className="w-6 h-6" />
                        </div>
                        <div>
                          <p className="font-semibold text-lg">Mercado Pago</p>
                          <p className="text-neutral-600">Tarjetas, PSE, Efecty</p>
                        </div>
                      </label>
                    </div>
                    {paymentMethod === 'mercadopago' && (
                ...
                {paymentMethod === 'mercadopago' && mpPublicKey && (
                  <div className="mt-6 p-6 bg-white rounded-xl border border-neutral-200 animate-fade-in">
                    <Payment
                      initialization={{ amount: total }}
                      onSubmit={async (param) => {
                        console.log(param);
                        alert('Pago recibido con éxito (Demo)');
                      }}
                      customization={{
                        paymentMethods: {
                          creditCard: 'all',
            ...
            <Card className="sticky top-6 shadow-xl border-0 bg-white/90 backdrop-blur-sm">
              ...
              <CardContent className="p-6 space-y-4">
                ...
                <Button 
                  ...
                  className={`w-full bg-neutral-900 hover:bg-neutral-800 text-white h-14 text-lg font-medium rounded-xl transition-all duration-300 transform hover:scale-[1.02] disabled:scale-100 disabled:opacity-70 ${paymentMethod === 'mercadopago' ? 'hidden' : ''}`}
                >
                  {isProcessing ? (
